...
class AiInsightGrid extends ConsumerStatefulWidget {
  const AiInsightGrid({super.key, required this.insights, this.showHeader = true});
...
class AiInsightGridState extends ConsumerState<AiInsightGrid> {
  ...
  Future<void> _generate(
      String id, StateSetter setSheet) async {
    ...
    try {
      ...
      final budget = BudgetStatus.evaluate(monthSpend,
          ref.read(appSettingsProvider).monthlyBudgetCents);
      final text = await ref.read(aiServiceProvider).insightByDimension(
          id, items, sales, widget.insights,
          budget: budget);
      await _save(id, text);
      if (mounted) {
        setState(() {
              ...
              children: [
                Padding(
                  ...
                  child: Row(
                    children: [
                      Icon(_icons[dim.id],
                          size: 20, color: AppTheme.greenLight),
                      const SizedBox(width: 8),
                      Expanded(
                        child: Text('AI 洞察 · ${dim.title}',
                    ...
                    child: _loading
                        ...
                        : text == null
                            ...
                            : MarkdownBody(
                                data: text,
                                styleSheet:
                                    digestMarkdownStyle(sheetContext),
                              ),
                  ),
                ),
                SafeArea(
