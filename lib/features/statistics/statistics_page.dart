...
class StatisticsPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (active.isEmpty) {
      return Scaffold(
        ...
        body: EmptyView(
          ...
          title: '还没有可分析的物品',
          subtitle: '记录物品后，这里会整理消费、使用价值与待处理事项',
          action: FilledButton.tonal(
            onPressed: () => context.push('/item/new'),
            child: const Text('去添加物品'),
          ),
        ),
      );
    }
    ...
    final idle = active.where((i) => i.status == ItemStatus.idle).toList();
    ...
    final highestDaily = StatisticsService.dailyCostRanking(
      active,
      sales,
      descending: true,
      limit: 5,
    );
    final lowestDaily = StatisticsService.dailyCostRanking(
      active,
      sales,
      descending: false,
      limit: 5,
    );
    final longest = StatisticsService.longestUsedRanking(
      active,
      sales,
      limit: 5,
    );
    final retention = StatisticsService.retentionRanking(
      active,
      sales,
      limit: 5,
    );
    ...
    final budget = BudgetStatus.evaluate(
      monthSpend,
      ref.watch(appSettingsProvider).monthlyBudgetCents,
    );
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          _section(context, '当前概览'),
          SceneCard(
            label: '当前拥有',
            value: '${overview.ownedCount}',
            unit: '件',
            subLabel: '持有购买 ',
            subValue: Money.format(
          ...
          GridView.count(
            ...
            children: [
              MetricTile(
                label: '转卖回收净收入',
                value: Money.formatCompact(
                  overview.saleNetIncomeTotal,
                  currency: currency,
                ),
              ),
              MetricTile(
                label: '历史实际损耗',
                value: Money.formatCompact(
                  overview.historyDepreciationTotal,
                  currency: currency,
                ),
              ),
              MetricTile(
          ...
          // 统计透视表（卡内自带紧凑标题，免与区块距叠加出空带）。
          const PivotTableCard(),
          const SizedBox(height: 10),
          _section(context, '需要处理'),
            ...
            Column(
              children: [
                ...
                ...categories.take(8).map((c) {
                  ...
                  final palette = treemapPalette(Theme.of(context).brightness);
                  return Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: Text('${c.categoryName} · ${c.count}件'),
                        ),
                        Text(
                          Money.formatCompact(
                            c.purchaseTotal,
          ...
          if (statusRows.length > 1) ...[
            _section(context, '物品状态'),
            _distCard(context, [
              for (final e in statusRows)
                _DistRow(
                  e.key.label,
                  e.value / statusRows.first.value,
                  '${e.value} 件',
                ),
            ]),
          ],
          ...
          _section(context, '近六个月'),
          // 折线图去盒：直接坐画布，描述行删除（轴标签已含月份）。
          Padding(
            padding: const EdgeInsets.symmetric(vertical: 4),
            child: Column(
                      ...
                      lineTouchData: LineTouchData(
                        ...
                        touchTooltipData: LineTouchTooltipData(
                          getTooltipItems: (spots) => spots
                              .map(
                                (e) => LineTooltipItem(
                                  Money.formatCompact((e.y * 100).round()),
                                  const TextStyle(
                                    fontSize: 12,
                        ...
                        LineChartBarData(
                          spots: trend
                              ...
                              .map(
                                (e) => FlSpot(
                                  e.key.toDouble(),
                                  (e.value.purchaseTotal / 100).toDouble(),
                                ),
                              )
                              .toList(),
                          isCurved: true,
            ...
            child: Column(
              children: [
                _rankTile(
                  ...
                  Icons.trending_up,
                  highestDaily,
                  (r) => '${(r.dailyCost / 100).toStringAsFixed(1)} 元/天',
                ),
                ...
                if (retention.isNotEmpty) ...[
                  ...
                  _rankTile(
                    ...
                    retention,
                    (r) => '${(r.value * 100).toStringAsFixed(0)}%',
                  ),
                ],
                if (idle.isNotEmpty) ...[
                  const Divider(height: 1, indent: 16, endIndent: 16),
          ...
          if (ref.watch(aiConfigProvider).isReady) ...[
            _section(context, 'AI 解读'),
            AiInsightGrid(insights: insights, showHeader: false),
          ],
        ],
      ),
    );
  ...
  double _niceInterval(List<MonthlyStats> trend) {
    final maxY = trend.fold<double>(
      0,
      (m, t) => m > t.purchaseTotal / 100 ? m : t.purchaseTotal / 100,
    );
    if (maxY <= 0) return 1;
    final raw = maxY / 3;
  ...
  Widget _section(BuildContext context, String title) {
    return Padding(
      ...
      child: Text(
        title,
        style: AppTheme.title(Theme.of(context).colorScheme.onSurface),
      ),
    );
  }
  ...
  Widget _rankTile(
    ...
    String title,
    IconData icon,
    List<ValueRanking> ranking,
    String Function(ValueRanking) valueText,
  ) {
    if (ranking.isEmpty) {
      return Padding(
        padding: const EdgeInsets.all(12),
        ...
        builder: (context, scrollController) => Column(
          children: [
            Padding(
              padding: const EdgeInsets.all(16),
              child: Text(
                '${entry.title}（${entry.items.length} 件）',
            ...
            Expanded(
              child: ListView(
                ...
                children: entry.items.map((i) => ItemCard(item: i)).toList(),
              ),
            ),
          ],
        ),
...
class _BudgetCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final statusText = switch (budget.level) {
      BudgetLevel.ok =>
        '剩余 ${Money.format(budget.remainingCents, currency: currency)}',
      BudgetLevel.warning =>
        '仅剩 ${Money.format(budget.remainingCents, currency: currency)}，接近预算',
      BudgetLevel.exceeded =>
        '已超支 ${Money.format(-budget.remainingCents, currency: currency)}',
    };
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
