...
class AiService {
  ...
  Future<AiItemDraft> parseItem(String input, List<Category> categories,
    ...
    final raw = imageBase64 == null
        ...
        : await client.askWithImage(system, userText, imageBase64,
            jsonMode: true);
  ...
  Future<String> weeklyReport(
    ...
    final saleIncome = soldThisWeek.fold<int>(
        0, (s, i) => s + (sales[i.id]?.netIncome ?? 0));
  ...
  Future<String> summarize(
    ...
    final catText = cats
        .take(8)
        .map((c) => '${c.categoryName} ${c.count}件共${(c.purchaseTotal / 100).toStringAsFixed(0)}元')
        .join('；');
    final idle =
        items.where((i) => !i.isDeleted && i.status == ItemStatus.idle).length;
    final data =
  ...
  Future<String> insightByDimension(
    ...
    final system = '${AiPrompts.systemBase}\n'
    ...
    try {
      ...
      final fullContext =
          extra.isEmpty ? context : '$context\n\n${extra.join('\n\n')}';
      return client.ask(
        system.replaceAll('你可以调用工具主动查询用户的本地物品数据（RAG）：'
      ...
      case 'query_items':
        {
          ...
          final body = buildItemRoster(matched, sales,
              ownedOnly: false, limit: limit);
  ...
  Future<String> chatQuery(
    ...
    final detail = retrieved.matched.isEmpty
        ? '（本次检索没有命中具体物品，请看统计摘要）'
        : retrieved.matched
            .map((i) => '- ${i.name}｜id:${i.id}｜分类:${i.categoryName}｜'
    ...
    if (actionHandler != null) {
      try {
        final insights =
            ItemInsightService.analyze(active, sales);
          ...
          'function': {
            ...
            'parameters': {
              ...
              'properties': {
                ...
                'locationName': {'type': 'string', 'description': '新位置名称（必须是已有位置的名称）'},
  ...
  static String buildItemRoster(
    ...
    final now_ = now ?? DateTime.now();
    final active = items.where((i) => !i.isDeleted).toList();
    final list = (ownedOnly ? active.where((i) => i.status.isOwned) : active)
        .toList()
  ...
  static String buildSoldRoster(
      List<Item> items, Map<String, SaleRecord> sales) {
    final sold = items
        .where((i) => !i.isDeleted &&
            i.status == ItemStatus.sold &&
            sales[i.id] != null)
  ...
  static String buildMonthlySummary(List<Item> items) {
    final trend = StatisticsService.monthlyTrend(
        items.where((i) => !i.isDeleted).toList(), const {},
        months: 6);
  ...
  Future<String> purchaseEvaluation({
    ...
    final dupText = dups.isEmpty
        ? '（没有相似物品）'
        : dups
            .map((m) => '- ${m.item.name}（${m.item.categoryName}，'
    ...
    if (categoryName != null) {
      final matched = active
          ...
          .toList();
      final spent =
          matched.fold<int>(0, (s, i) => s + i.purchasePrice);
      categorySpend = '「$categoryName」已有 ${matched.length} 件、共花 '
    ...
    if (budgetCents > 0) {
      ...
      final afterPct = ((monthSpend + priceCents) / budgetCents * 100)
          .toStringAsFixed(0);
      budgetLine = '月度预算 ${(budgetCents / 100).toStringAsFixed(0)} 元，'
  ...
  Future<Map<String, List<String>>> generateItemTags(List<Item> items) async {
    ...
    final lines = items
        .map((i) =>
            '${i.id}｜${i.name}｜${i.brand ?? '-'}｜${i.categoryName}')
        .join('\n');
    final raw = await client.ask(AiPrompts.kTaggingSystem, lines,
        jsonMode: true);
  ...
  Future<String> yearlyReport(
    ...
    final topItems = [...inYear]..sort((a, b) => b.purchasePrice.compareTo(a.purchasePrice));
    ...
    final lines = [
      ...
      if (idle.isNotEmpty) '这一年购买但现已闲置：${idle.length} 件（${idle.take(5).map((i) => i.name).join('、')}）。',
  ...
  static String buildInsightContext(
    ...
    return [
      ...
      if (idleNames.isNotEmpty) '长期闲置（${insights.longIdle.length} 件）：$idleNames。',
      ...
      if (budget != null)
        '月度预算：本月已花费 ${(budget.spentCents / 100).toStringAsFixed(0)} 元 / '
            ...
            '（${(budget.ratio * 100).toStringAsFixed(0)}%${budget.level == BudgetLevel.exceeded ? '，已超支' : budget.level == BudgetLevel.warning ? '，接近预算' : ''}）。',
    ].join('\n');
  }
