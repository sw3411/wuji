...
class ItemRepository {
  ...
  Stream<List<Item>> watchAll() {
    return _db.select(_db.items).watch().map(
        (rows) => rows.map(AppDatabase.toItem).toList());
  }

  Stream<Item?> watchById(String id) {
    return (_db.select(_db.items)..where((t) => t.id.equals(id)))
        .watch()
        .map((rows) => rows.isEmpty ? null : AppDatabase.toItem(rows.first));
  }

  Future<Item?> getById(String id) async {
    final rows = await (_db.select(_db.items)..where((t) => t.id.equals(id))).get();
  ...
  Future<void> updateItem(Item item) async {
    await (_db.update(_db.items)..where((t) => t.id.equals(item.id)))
        .write(AppDatabase.toItemsCompanion(item));
  }
  ...
  Future<void> restore(String id) async {
    await (_db.update(_db.items)..where((t) => t.id.equals(id)))
        .write(const ItemsCompanion(deletedAt: Value(null)));
  }
  ...
  Future<void> batchMoveLocation(List<String> ids, String? locationId, String? locationName) async {
    for (final id in ids) {
      final item = await getById(id);
      if (item == null) continue;
      await updateItem(locationId == null
          ? item.copyWith(clearLocation: true)
          : item.copyWith(locationId: locationId, locationName: locationName));
      if (locationId != null) {
        await addEvent(ItemEvent(
          id: '',
          itemId: id,
  ...
  Future<void> batchChangeCategory(List<String> ids, String categoryId, String categoryName) async {
    for (final id in ids) {
      final item = await getById(id);
      if (item == null) continue;
      await updateItem(item.copyWith(categoryId: categoryId, categoryName: categoryName));
    }
  }
  ...
  Future<void> addEvent(ItemEvent event) async {
    await _db.into(_db.itemEvents).insert(ItemEventsCompanion.insert(
          id: event.id.isEmpty
